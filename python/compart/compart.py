"""Compart - kernel-level sandbox with an insulated runtime for isolated execution.
...
* :class:`AgentCompart` — an agent outer compartment (pro tier). It auto-loads every
...
_logger = logging.getLogger("compart")
...
class Compart:
    ...
    def __init__(
        self,
        workdir: str = ".",
        config: Optional[CompartConfig] = None,
        verbose: bool = False,
    ):
        if config is not None:
            self.config = config
        else:
            self.config = CompartConfig(workdir=workdir)
        self.workdir = os.path.abspath(self.config.workdir)
        self._snapshot_base = os.path.join(self.workdir, self.COMPART_DIR, "snapshots")
    ...
    def add(self, compartment: Compartment) -> "Compart":
    ...
    def edge(self, from_name: str, to_name: str) -> "Compart":
    ...
    def register_module(self, module_cls) -> "Compart":
    ...
    def run(
        ...
        CompartResult
        """
        task_desc = request or entry or "compart.run()"
        self._started_at = time.time()
        _logger.info("Compart %s running: %s", self.box_id, task_desc[:80])
    ...
    def _build_result(self, raw: dict[str, Any], elapsed: float) -> CompartResult:
        errors: list[str] = []
        completed: list[str] = []
        status = "success"
...
class AgentCompart(Compart):
    ...
    def __init__(
        ...
        if config is not None:
            ...
            config = CompartConfig(workdir=workdir, auto_modules=True)
