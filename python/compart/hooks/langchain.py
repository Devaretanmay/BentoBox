...
class CompartPythonREPLTool(BaseTool):  # type: ignore[misc]
    ...
    description: str = (
        ...
        "Runs inside a kernel-enforced Compart sandbox."
    )
...
class CompartGraphNode:
    """Wrap a function so it runs inside a Compart compartment.

    Turns any callable into a LangGraph node that executes within a
    kernel-enforced :class:`compart.AgentCompart`. The wrapped node calls
    ...
    def _permissions_from(self, metadata: Optional[Mapping[str, Any]]) -> tuple[str, ...]:
        ...
        if metadata:
            listed = metadata.get("permissions") or metadata.get("compart_permissions")
            if listed:
    ...
    def attach(
        ...
        *,
        ...
        if builder is None:
            raise TypeError(
                "CompartGraphNode.attach requires langgraph; install it with"
                " 'pip install langgraph'."
            )
        node_name = name or getattr(self._fn, "__name__", "compart_node")
