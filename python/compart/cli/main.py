...
_ACTIVATE_TEMPLATE = """\
#!/usr/bin/env bash
# Compart workspace activator
# Usage: source .compart/activate
#
# Prepends .compart/bin to PATH so workspace shims intercept agent commands.

_COMPART_BIN="$(cd "$(dirname "${BASH_SOURCE[0]}")/bin" && pwd)"
if [[ ":$PATH:" != *":$_COMPART_BIN:"* ]]; then
  export PATH="$_COMPART_BIN:$PATH"
  echo "Compart workspace activated. Type 'deactivate_compart' to undo."
fi

deactivate_compart() {
  export PATH="${PATH//$_COMPART_BIN:/}"
  unset -f deactivate_compart
  echo "Compart workspace deactivated."
}
"""

_ENVRC_TEMPLATE = """\
# Compart workspace — direnv integration
# Generated by `compart init`. Requires https://direnv.net
PATH_add .compart/bin
"""
...
def _resolve_real_binary(agent_name: str, workspace_root: str) -> str | None:
    ...
    When the workspace is activated, ``.compart/bin`` is prepended to PATH, so
    ``shutil.which(agent)`` would return our own shim script and recurse
    forever. Resolve against the rest of PATH instead so the supervisor always
    launches the genuine agent binary.
    """
...
def cmd_init(args):
    ...
    activate_path = os.path.join(COMPART_DIR, "activate")
    with open(activate_path, "w", encoding="utf-8") as f:
        f.write(_ACTIVATE_TEMPLATE)
    os.chmod(activate_path, os.stat(activate_path).st_mode | stat.S_IEXEC)

    envrc_path = ".envrc"
    if not os.path.exists(envrc_path):
        with open(envrc_path, "w", encoding="utf-8") as f:
            f.write(_ENVRC_TEMPLATE)
    ...
    print()
    print("┌────────────────────────────────────────────────────────────────┐")
    print("│  Run this to activate (one time per terminal session):       │")
    print("│                                                              │")
    print("│    source .compart/activate                                  │")
    print("│                                                              │")
    print("│  Or use direnv for automatic activation: direnv allow        │")
    print("└────────────────────────────────────────────────────────────────┘")
