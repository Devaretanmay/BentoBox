...
or LLM-authored code inside a :class:`bentoworks.AgentBentoBox` compartment.
...
:class:`bentoworks.sandbox.enforcer.SandboxEnforcer` still enforces the
...
_logger = logging.getLogger("bentoworks.hooks")
...
class ExecutionResult:
    """Structured outcome of running code inside a BentoBox compartment.
...
class SandboxRunner:
    """Execute shell commands or code snippets inside an AgentBentoBox.

    A fresh :class:`bentoworks.AgentBentoBox` is created per :meth:`run`
    ...
    credential_rules
        ...
        :class:`bentoworks.sandbox.proxy.RouteConfig`).
    sandbox
    ...
    def _box(self) -> AgentBentoBox:
        return AgentBentoBox(config=BentoBoxConfig(
    ...
    def run_code(
        ...
        *,
        ...
        The snippet is written under ``.bentoworks/tmp`` (outside the diff
        ...
        tmp_root = os.path.join(self.workdir, ".bentoworks", "tmp")
