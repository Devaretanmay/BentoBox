...
import compart from '../index.js'

assert.ok(typeof compart.version() === 'string' && compart.version().length > 0, 'version() returns a string')
console.log('version:', compart.version())

assert.ok(typeof compart.sandboxSupported() === 'boolean', 'sandboxSupported() returns boolean')
console.log('sandboxSupported:', compart.sandboxSupported())

const original = 'the quick brown fox jumps over the lazy dog. '.repeat(10)
const compressed = compart.compress(original)
...
assert.ok(compart.runtimeCheckPermission(policy, JSON.stringify(['fs_read'])) === true, 'fs_read allowed')
assert.ok(compart.runtimeCheckPermission(policy, JSON.stringify(['network'])) === false, 'network denied')
console.log('runtimeCheckPermission: allowed/denied ok')

assert.ok(compart.runtimeCheckCommand('rm -rf /') === false, 'rm -rf / blocked')
assert.ok(compart.runtimeCheckCommand('grep foo file.txt') === true, 'grep allowed')
console.log('runtimeCheckCommand: blocklist ok')

const configs = JSON.stringify({ configs: [{ name: 'a', allow_outbound_to: ['b'] }, { name: 'b' }] })
assert.ok(compart.runtimeValidate(configs, JSON.stringify([['a', 'b']])) === true, 'runtime valid')
assert.ok(compart.runtimeValidate(configs, JSON.stringify([['a', 'nope']])) === false, 'runtime invalid')
console.log('runtimeValidate: ok')

assert.ok(compart.runtimeCanRoute(configs, 'a', 'b') === true, 'a->b allowed')
assert.throws(() => compart.runtimeCanRoute(configs, 'zzz', 'b'), 'unknown compartment throws')
...
const rt = new compart.Runtime(handleConfigs, JSON.stringify([['a', 'b']]))
...
assert.ok(compart.runtimeCredentialRewrite(routes, '/openai/v1/chat') === 'https://api.openai.com/v1/chat', 'route rewrite')
assert.ok(compart.runtimeCredentialRewrite(routes, '/anthropic/v1') === null, 'no match -> null')
console.log('runtimeCredentialRewrite: ok')

process.env.BW_TS_TEST_KEY = 'sk-test'
try {
  assert.ok(compart.runtimeCredentialResolve('env:BW_TS_TEST_KEY') === 'sk-test', 'env credential resolve')
  assert.ok(compart.runtimeCredentialResolve('env:BW_TS_MISSING') === '', 'missing env -> empty')
} finally {
...
assert.ok(compart.runtimeSnapshot(snapWork, snapDir) === 1, 'snapshot count 1')
writeFileSync(`${snapWork}/a.txt`, 'changed')
assert.ok(compart.runtimeRestore(snapWork, snapDir) === 1, 'restore count 1')
