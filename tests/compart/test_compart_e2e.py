"""E2E tests for the compartment-centric Compart API."""
...
class TestCompartInit(unittest.TestCase):

    def setUp(self):
        self.tmpdir = os.path.join(tempfile.gettempdir(), f"compart_e2e_{uuid.uuid4().hex[:8]}")
    ...
    def test_plain_box_loads_no_modules_by_default(self):
        """Compart is just the box: no modules auto-load."""
        box = AgentCompart(workdir=self.tmpdir)
        ...
        self.assertGreater(module_count, 0, "AgentCompart should auto-load modules")

        plain = Compart(workdir=self.tmpdir)
        plain._box.insulate("Fix the bug")
        plain_count = sum(len(e.modules) for e in plain._box._engines.values())
        self.assertEqual(plain_count, 0, "plain Compart ships empty")
...
class TestCompartmentFlow(unittest.TestCase):
    ...
    def setUp(self):
        self.tmpdir = os.path.join(tempfile.gettempdir(), f"compart_flow_{uuid.uuid4().hex[:8]}")
        _make_repo(self.tmpdir)
